...
const ALLOWED_ROLES = ["admin", "super_admin", "operator", "accounting", "billing"];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[AdminCreateUser] Creazione utente autorizzata: ${normalizedEmail}, Ruolo: ${role}`);
    ...
    if (createError) {
      console.error(`[AdminCreateUser] Errore Auth: ${createError.message} (Code: ${createError.code || "n/a"})`);
    ...
    console.log(`[AdminCreateUser] Successo! UUID: ${user?.id}. Il trigger handle_new_user popola public.users.`);
