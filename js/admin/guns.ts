...
export async function showGunsModal(
  islandId: number,
  islandName: string,
...
async function renderGuns(
  ...
  try {
    const { data: rawGuns, error } = await safeSupabaseQuery(
      async () => await supabase.from('pistole').select('*').eq('island_id', islandId).order('nome')
    );
        ...
        children: [
          ...
          createEl('button', {
            ...
            children: [
              ...
              document.createTextNode(' Aggiungi Prima Pistola')
            ]
          })
        ]
      });
      emptyState
        .querySelector('.fa-gas-pump')
        ?.setAttribute('style', 'font-size: 3rem; margin-bottom: 15px; opacity: 0.3;');
    ...
    guns.forEach(gun => {
      ...
      header
        .querySelector('.fa-gas-pump')
        ?.setAttribute('style', `color: ${color}; margin-right: 8px;`);
      ...
      const counterBox = createEl('div', {
        ...
        children: [
          createEl('div', {
            ...
            children: [
              createIcon('fas fa-tachometer-alt'),
              document.createTextNode(' Numeratore Attuale')
            ]
          }),
          createEl('div', {
    ...
    target.querySelectorAll('.delete-gun').forEach(btn => {
      ...
      b.addEventListener('click', () => {
        const id = b.dataset.id;
        if (id) {
          deleteGun(parseInt(id, 10), islandId, islandName, stationId);
        }
      });
    });
...
async function openGunForm(
  islandId: number,
  islandName: string,
  stationId: number,
  gunId: number | null = null
): Promise<void> {
  const isEdit = !!gunId;
  ...
  if (isEdit && gunId) {
    const { data, error } = await safeSupabaseQuery(
      async () => await supabase.from('pistole').select('*').eq('id', gunId).single()
    );
    if (error) {
      throw error;
    }
  ...
  const tipoSelect = createEl('select', {
    attrs: { name: 'tipo_carburante', required: 'required' }
  });
  const benzinaOption = createEl('option', { attrs: { value: 'benzina' }, text: 'Benzina' });
  if (gun.tipo_carburante === 'benzina') {
    benzinaOption.selected = true;
  }
  const gasolioOption = createEl('option', { attrs: { value: 'gasolio' }, text: 'Gasolio' });
  if (gun.tipo_carburante === 'gasolio') {
    gasolioOption.selected = true;
  }
      ...
      try {
        if (isEdit && gunId) {
          const { error: updateError } = await safeSupabaseQuery(
            async () => await supabase.from('pistole').update(payload).eq('id', gunId)
          );
          if (updateError) {
            throw updateError;
          }
          ...
          const { error: insertError } = await safeSupabaseQuery(
            async () => await supabase.from('pistole').insert([payload])
          );
          if (insertError) {
            throw insertError;
          }
...
async function showCounterEditModal(
  gunId: number,
  gunName: string,
    ...
    children: [
      createEl('div', {
        ...
        children: [
          ...
          document.createTextNode(
            ' Modificare il numeratore influenzerà i calcoli delle chiusure future.'
          )
        ]
      })
    ]
...
async function deleteGun(
  ...
  try {
    ...
    const { error: deleteError } = await safeSupabaseQuery(
      async () => await supabase.from('pistole').delete().eq('id', gunId)
    );
    if (deleteError) {
      throw deleteError;
    }
