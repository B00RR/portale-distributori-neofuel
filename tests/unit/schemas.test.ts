...
describe('Zod Schemas (real validation)', () => {
    describe('LoginSchema', () => {
        ...
        it('rejects an email with surrounding whitespace (real behavior: .email() runs before .trim())', () => {
            ...
            expect(safeParse(LoginSchema, { email: '  user@example.com  ', password: 'secret1' }).success).toBe(false);
        });
    ...
    describe('CreateUserSchema', () => {
        const base = { email: 'a@b.com', password: '123456', full_name: 'Mario Rossi', role: 'operator' as const };
        ...
        it('rejects a too-short full_name and a too-long one (boundaries)', () => {
            ...
            expect(safeParse(CreateUserSchema, { ...base, full_name: 'A'.repeat(101) }).success).toBe(false);
        });
    });

    describe('UpdateUserSchema', () => {
        it('accepts valid data and rejects an invalid role', () => {
            expect(safeParse(UpdateUserSchema, { full_name: 'Mario Rossi', role: 'admin' }).success).toBe(true);
            expect(safeParse(UpdateUserSchema, { full_name: 'Mario Rossi', role: 'nope' }).success).toBe(false);
        });
    });
    ...
    describe('PriceUpdateSchema', () => {
        ...
        it('coerces a string station_id to a number', () => {
            ...
            expect(res.success).toBe(true);
            if (res.success) { expect(res.data.station_id).toBe(42); }
        });
    ...
    describe('ShiftIdSchema', () => {
        it('accepts a numeric id and coerces a string id', () => {
            ...
            expect(res.success).toBe(true);
            if (res.success) { expect(res.data.id).toBe(7); }
        });
    });
    ...
    describe('BulkExportSchema', () => {
        it('accepts last_n and applies the default limit', () => {
            ...
            expect(res.success).toBe(true);
            if (res.success) { expect(res.data.limit).toBe(10); }
        });

        it('rejects a date_range without both dates (refine), accepts it with both', () => {
            expect(safeParse(BulkExportSchema, { stationId: null, type: 'date_range' }).success).toBe(false);
            expect(safeParse(BulkExportSchema, {
                stationId: null, type: 'date_range', dateFrom: '2026-01-01', dateTo: '2026-01-31'
            }).success).toBe(true);
        });

        it('rejects a limit out of [1,100] (boundaries)', () => {
            expect(safeParse(BulkExportSchema, { stationId: null, type: 'last_n', limit: 0 }).success).toBe(false);
            expect(safeParse(BulkExportSchema, { stationId: null, type: 'last_n', limit: 101 }).success).toBe(false);
            expect(safeParse(BulkExportSchema, { stationId: null, type: 'last_n', limit: 100 }).success).toBe(true);
        });
    });
    ...
    describe('AssignStationSchema', () => {
        ...
        it('rejects a bad uuid and a non-positive station id', () => {
            expect(safeParse(AssignStationSchema, { user_id: 'not-a-uuid', station_id: 3 }).success).toBe(false);
    ...
    describe('helpers', () => {
        it('safeParse returns a joined error string on failure', () => {
            ...
            expect(res.success).toBe(false);
            if (!res.success) { expect(typeof res.error).toBe('string'); expect(res.error.length).toBeGreaterThan(0); }
        });

        it('parse throws on invalid input and returns data on valid input', () => {
            expect(() => parse(LoginSchema, { email: 'bad', password: '1' })).toThrow();
            expect(parse(LoginSchema, { email: 'a@b.com', password: '123456' })).toMatchObject({ email: 'a@b.com' });
        });
