...
describe('Utils Module', () => {
  ...
  describe('Parsing', () => {
    ...
    it('parseGunCounter should preserve decimal points (#247)', () => {
      expect(parseGunCounter('1234.56')).toBe(1234.56);
      expect(parseGunCounter('1,234.56')).toBe(1234.56);
    });

    it('localized parsers should reject malformed grouping', () => {
      expect(parseGunCounter('1.2.3')).toBe(0);
      expect(parseGunCounter('12.34,56')).toBe(0);
      expect(parseNumberFlexible('not-a-number')).toBe(0);
    });
    ...
    it('parseNumberFlexible should handle various inputs', () => {
      ...
      expect(parseNumberFlexible('1 234,56')).toBe(1234.56);
      expect(parseNumberFlexible('1.234.567')).toBe(1234567);
