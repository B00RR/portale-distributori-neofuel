...
vi.mock('../../js/operator/extra-income.js', () => ({
  processExtraIncome: mockProcessExtraIncome
}));
...
describe('Offline financial executors bootstrap', () => {
  ...
  it('replays queued outflows without queueing them again', async () => {
    ...
    expect(result).toBe(true);
    expect(mockProcessOutflow).toHaveBeenCalledWith(
      123,
      '456',
      15,
      'prelievo',
