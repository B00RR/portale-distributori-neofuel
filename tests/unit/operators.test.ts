...
describe('Operators Module', () => {
    ...
    beforeEach(async () => {
        ...
        vi.doMock('../../js/core/schemas.js', () => ({
            CreateUserSchema: {},
            UpdateUserSchema: {},
            safeParse: vi.fn().mockReturnValue({ success: true, data: { full_name: 'Test', role: 'operator' } })
        }));
    ...
    it('should handle Operator Creation (RPC via Edge Function)', async () => {
        ...
        await vi.waitFor(() => {
            expect(supabase.functions.invoke).toHaveBeenCalledWith('admin_create_user_v2', expect.anything());
    ...
    it('should handle Operator Edit', async () => {
        const mockUser = { user_id: 'u1', full_name: 'Old Name', email: 'old@test.com', role: 'operator' };
