  ...
  () => ({
    ...
    mockUIComponents: {
      ...
      createFormActions: vi.fn(
        opts => `<div><button id="btn-cancel">${opts.confirmText || 'Confirm'}</button></div>`
      )
    },
    mockOpening: {
    ...
    mockLogger: {
      ...
      getUserMessage: vi.fn(
        (errorId: string) => `Si è verificato un errore. Riferimento: ${errorId}`
      )
    }
  })
);
...
describe('Operator Outflows Module', () => {
  ...
  describe('showOutflowMenu', () => {
    ...
    it('should submit form and save to database', async () => {
      ...
      expect(mockSupabase.rpc).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          p_station_id: expect.any(Number),
          p_shift_id: 1,
          p_tipo: 'uscita',
          p_importo: 75.5
        })
    ...
    it('should handle and log database errors', async () => {
      mockOpening.checkOpeningStatus.mockResolvedValue({ id: 1 });
      mockSupabase.rpc.mockResolvedValue({ data: null, error: { message: 'Insert failed' } });
