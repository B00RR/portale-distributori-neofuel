...
- Il pannello admin (`js/admin/`) è accessibile solo agli utenti con ruolo `admin` o `super_admin`.
...
- `public.is_admin()` → verifica che il ruolo sia `admin` o `super_admin`.
...
2. Non rimuovere mai controlli di ruolo da funzioni di autorizzazione: se `is_admin()` controlla `admin` **e** `super_admin`, mantenere entrambi.
